...
- **No target** → ask user via asking the user: "What should I review? (file, directory, or --diff for changed files)"
...
□ If yes and --diff not specified: offer via asking the user:
...
FOR each active role (order: Developer → Security Expert → Architect → [Ops/SRE]):
    ...
    □ Create task tracking entry: "[Code Review] <Role> scanning"
