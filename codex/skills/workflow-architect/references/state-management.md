...
Added by the `bug-fixer` add-on skill. Initialized on first Bug Fixer invocation (not at state.json creation).
...
Added by the `issue-changer` add-on skill. Initialized on first Issue Changer invocation (not at state.json creation).
