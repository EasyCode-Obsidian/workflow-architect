...
This skill **requires** `.workflow/<name>/state.json` to exist. Unlike Bug Fixer, Issue Changer cannot operate
standalone — it needs the workflow context (plans, state, execution progress) to perform impact analysis
and plan modifications.

If `.workflow/` does not exist (or has no instances): inform the user that this skill requires an active or completed
...
On invocation, check for `.workflow/` directory. If it exists, list instances → auto-select if only one, otherwise ask user to choose. Then read `.workflow/<name>/state.json` and determine the mode:
...
2. Scan `.workflow/<name>/phases/` plan files → identify affected tasks
