...
**If inside a git repository**, ask the user for scan mode selection:
...
<!-- 过时依赖、已知 CVE、许可证冲突。优先使用原生审计工具，web search tool 仅作回退。 -->
...
- Fall back to manual checks:
  ...
  2. Use the web search tool to check the top 5 most critical dependencies (by centrality) for known CVEs
