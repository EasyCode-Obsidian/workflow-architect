...
**If inside a git repository**, offer scan mode selection via `AskUserQuestion`:
...
<!-- 过时依赖、已知 CVE、许可证冲突。优先使用原生审计工具，WebSearch 仅作回退。 -->
...
- Fall back to manual checks:
  ...
  2. Use `WebSearch` to check the top 5 most critical dependencies (by centrality) for known CVEs
  3. Mark all WebSearch-sourced findings as `⚠️ WebSearch fallback — verify manually`
...
When triggered from the parent project-surgeon:
