...
> Handles change requests after the project-surgeon project has completed.
...
Use `AskUserQuestion` for each question. Max 5 questions — do NOT enter the full 10-category taxonomy.
...
- Severity classification still applies (light/moderate/major):
  ...
  - **Major**: Architecture change needed → warn user this may be better suited for a new `/project-surgeon` project

**For major changes:** Present the option to start a fresh project-surgeon project instead:
> "This change is significant enough that starting a new project with `/project-surgeon` might produce better results. Would you prefer to: (A) Proceed with incremental change, or (B) Start a new project that incorporates this change?"

## Step 4: Incremental Planning — 增量规划

<!-- 为变更创建独立的增量执行计划。 -->

Create change-specific plans in `.project-surgeon/changes/change-N/`:

### Directory Structure

```
.project-surgeon/changes/change-<N>/
...
1. Create TaskCreate entries for all change tasks: `[Change #N] Task NN: <name>`
...
4. Update state.json:
   ...
   - `change_requests[N].resolution.plan_dir: ".project-surgeon/changes/change-N/"`
...
2. Gets its own `.project-surgeon/changes/change-N/` directory
