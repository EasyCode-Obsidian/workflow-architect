...
Step 2: SCAN PLANS — identify affected plan files in .project-surgeon/
...
3. If the description is ambiguous, ask clarifying questions via `AskUserQuestion` (max 2 questions)
...
1. Use `Grep` to search the project source code (excluding `.project-surgeon/`, `node_modules/`, etc.) for scope keywords
