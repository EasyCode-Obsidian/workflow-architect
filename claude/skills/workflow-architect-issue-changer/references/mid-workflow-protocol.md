...
1. **User explicitly invokes** `/workflow-architect-issue-changer` during Phase 4 execution
...
| Explicit invocation (显式调用) | +10 | `/workflow-architect-issue-changer` |
...
Score ≤ 2  →  LOW CONFIDENCE
                ...
                After the current task completes, append a non-intrusive hint:
                  ...
                  可以使用 /workflow-architect-issue-changer。"
```
...
|-------------|-------|-------|
| `/workflow-architect-issue-changer add notifications` | +10 | HIGH |
...
3. After approval:
   - Edit the task plan files in `.workflow/<name>/phases/` using `Edit` tool
...
3. Read `.workflow/draft-cache.md` to restore draft context
